...
/**********************************************************************************************************
*函 数 名: Pid_Cascade_Cal
*功能说明: 双环串级pid
*形    参: outside              inside  外环和内环pid结构体
  *        feedback_outside     feedback_inside 外环和内环反馈值
           pn_1                 pn_2            外环和内环输出正负调整
*返 回 值: PID反馈计算输出值
**********************************************************************************************************/
short Pid_Cascade_Cal(Pid_Typedef *outside, Pid_Typedef *inside, 
                      float feedback_outside , float feedback_inside,
                      char pn_1, char pn_2)
{
    int inttoshort;
    short result;
    outside->ActualValue = feedback_outside;
    inside->SetPoint = pn_1*PID_Calc(outside);
    inside->ActualValue = feedback_inside;
    inttoshort = pn_2*PID_Calc(inside);
    result = (short)inttoshort;
    return result;
}
/**********************************************************************************************************
...
float FeedForward_Cal(FeedForward_t *FF,float NowVal) //没有先验，难调，不建议使用
{
	FF->val_ddot = NowVal - FF->val - FF->val_dot;
	FF->val_dot = NowVal - FF->val;
	FF->val = NowVal;
	float val_dot_out = FF->val_dot*FF->param[1];
	
	FF->Out = FF->val*FF->param[0] + FF->val_dot_out_last*FF->val_dot_out_RC + val_dot_out*(1-FF->val_dot_out_RC) + FF->val_ddot*FF->param[2];
	
	FF->val_dot_out_last = val_dot_out;
	return LIMIT_MAX_MIN(FF->Out,FF->OutMax,-FF->OutMax);
}
