...
*返 回 值: 无
**********************************************************************************************************/
void set_powersate(Power_Limit_type *power_limit,SuperPower *super_power,int control_powerstate){
	
	if(control_powerstate == HalfCAP)
    {
        if(superpower.CAP_AD_H+0.5 < superpower.actual_vol && powerlimit.Min_capVol-0.5 < superpower.actual_vol)//当前电压大于飞坡所需最小的电容电压才能开启
...
void PowerLimit(Power_Limit_type *power_limit,int powerstate_cur){
  ...
  power_limit->predict_power = RM3508_R * (i_2) + power_limit->k_dynamic * (w_i) + power_limit->predict_p0;
	...
	if(power_limit->predict_power>0){
		power_limit->k_dynamic = (double)(LowPass_SetK(power_limit->k_dynamic,
	...
	a=0,b=0,c=0;
	for(int i=0;i<4;i++){
		if(powerlimit.Motor_lost[i]==1)
			continue;
		Pid_Typedef P = (power_limit->pidChassisWheelSpeed[i]);
		a += (double)RM3508_R * P.SetPoint * P.SetPoint * (power_limit->K[i]) * (power_limit->K[i]);
		b += (double)P.SetPoint * (2*RM3508_R*power_limit->K[i]*power_limit->M[i] + power_limit->k_dynamic*power_limit->K[i]*power_limit->Speed_Now[i]);
		c += (double)(RM3508_R*power_limit->M[i]*power_limit->M[i] + power_limit->k_dynamic*power_limit->M[i]*power_limit->Speed_Now[i]);
	}
	#if Mecanum == 2 
	//舵轮功率预测增加舵角电机
		for(int i=0;i<4;i++){
