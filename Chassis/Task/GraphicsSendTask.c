...
graphic_data_struct_t* Line_Draw(uint8_t layer,int Op_Type,uint16_t startx,uint16_t starty,uint16_t endx,uint16_t endy, uint16_t line_width, int color,uint8_t name[])
{
	static graphic_data_struct_t drawing;//必须为静态变量，否则函数结束时会销毁该变量内存
	memcpy(drawing.graphic_name,name,3);																			//图案名称，3位
	drawing.layer = layer;//层次
	drawing.operate_tpye = Op_Type;//操作类型
	drawing.graphic_tpye = TYPE_LINE;//图形类型
	drawing.width = line_width;//线宽
	drawing.color = color;//颜色
	drawing.start_x=startx;//起始点
	drawing.start_y=starty;
	drawing.end_x=endx;//终点
	drawing.end_y=endy;
	return &drawing;
}
...
//	static uint16_t startX, startY, endX, endY;
	...
	if(angle1 <  0)
		angle1 +=360;
	uint8_t optype = Init_Cnt == 0?Op_Change:Op_Add;//初始化完了就是change类型，不然就是添加图形
//	//车身线
//	P_graphic_data = Line_Draw(0,optype,centerx + len * (- arm_cos_f32(angle) - arm_sin_f32(angle)),
...
//	P_graphic_data = Line_Draw(0,optype,centerx + len*0.8 * (+ arm_cos_f32(angle) + arm_sin_f32(angle)),
//										centery + len*0.8 * (- arm_cos_f32(angle) + arm_sin_f32(angle)),
//										centerx + len*0.8 * (- arm_cos_f32(angle) + arm_sin_f32(angle)),
//										centery + len*0.8 * (- arm_cos_f32(angle) - arm_sin_f32(angle)),4 , Yellow , LaneLineName5);
//	memcpy(data_pack , (uint8_t*)P_graphic_data ,DRAWING_PACK);
